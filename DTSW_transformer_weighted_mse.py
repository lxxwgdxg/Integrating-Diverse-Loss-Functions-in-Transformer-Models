...
class WeightedMSELoss(nn.Module):
    def __init__(self, weight=2.0, size_average=True):
        super(WeightedMSELoss, self).__init__()
        self.weight_value = weight
        self.size_average = size_average

    def forward(self, inputs, targets, threshold=None):
        # Ensure weights are on the same device as targets
        self.weight = torch.tensor([self.weight_value], dtype=torch.float32).to(targets.device)
        one_tensor = torch.tensor([1.0], dtype=torch.float32).to(targets.device)
        
        # If no threshold is provided, use the mean of the targets
        if threshold is None:
            threshold = targets.mean().item()
        
        # Assign higher weights to values above the threshold
        weights = torch.where(targets > threshold, self.weight, one_tensor)
        ...
        # Assign higher weights to values above the threshold
        weights = torch.where(targets > threshold, self.weight, 1.0)
